...
- Swing 界面提供表单与表格联动，支持教材（含价格）、出版社、教材类型、订购、入库/领用的录入与查询，并在教材列表中展示教材类型与价格。
- 可独立打包成可执行 JAR，便于分发运行。
- 内置 ISBN 规则校验（必须以 `ISBN` 开头并跟随 10 位数字），触发器自动维护库存，存储过程输出订购/到货/发放统计。
- 采用 SwingWorker 异步加载列表数据，启动与刷新过程不会阻塞界面线程，保持窗口快速响应。
...
## 功能模块速览
- **教材维护**：录入/修改教材信息，ISBN 需满足 `ISBNXXXXXXXXXX` 格式；库存非负；支持录入教材价格；支持一键刷新同步数据库中的最新数据。
  新增教材类型与价格列，方便在表格中直接查看分类与定价。
- **出版社管理**：维护出版社名称与联系方式，可在教材表单中直接关联。
- **教材类型管理**：维护教材分类并可在教材表单中选择绑定。
- **教材订购**：下拉选择教材（显示名称+ISBN），填写订购/到货数量与状态，日期通过 `yyyy-MM-dd` 选择器录入，点击刷新按钮即可同步数据库最新教材列表。
- **入库与领用**：下拉选择教材（显示名称+ISBN），通过 “IN/OUT” 方向记录入库或领用，录入日期，触发器会自动同步教材库存，列表刷新后即可看到最新教材；出库操作会预检剩余库存，防止库存被扣减到 0。

## 库表、规则与触发器
- 表结构：`publishers`、`textbook_types`、`textbooks`、`textbook_orders`、`inventory_transactions`，均由 `DatabaseUtil` 启动时自动创建并填充示例数据。
- 约束：`textbooks.isbn` 启用格式检查，同时在业务层二次校验必须以 `ISBN` 开头且后续 10 位为数字；库存列强制非负，价格列默认为 0；业务层拦截导致库存减少到 0 的出库请求。
